...
const themeClasses = {
  fine: {
    ...
    band: "bg-[#0b0b0a]",
    card: "border-white/10 bg-[#10100e]",
            ...
            {demo.highlights.map((item, index) => (
              <div key={item} className={`border p-5 ${theme.card}`}>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black leading-tight text-[#2a170f]">{item}</h3>`r`n                    <p className="mt-2 font-sans text-sm font-semibold leading-6 text-[#6a4732]">
...
function BurgerButton({
  ...
  return (
    <a
      href={href}
      className={`inline-flex min-h-12 items-center justify-center border-[3px] border-[#15100c] px-6 py-3 text-center text-sm font-black uppercase shadow-[6px_6px_0_#15100c] transition hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[8px_8px_0_#15100c] ${variants[variant]}`}
    >
      {children}
    </a>
  );
...
function BurgerIntegratedDemo({ demo }: { demo: IntegratedDemo }) {
  return (
    <main className="bg-[#120b08] text-[#fff7e4] [font-family:Impact,Inter,ui-sans-serif,system-ui,sans-serif]">
      <nav className="sticky top-0 z-40 flex flex-wrap items-center justify-between gap-4 border-b-4 border-[#15100c] bg-[#ffcf2e] px-5 py-3 text-[#15100c] lg:px-[max(18px,calc((100vw-1120px)/2))]">
        <Link href="/#demos" className="flex items-center gap-3 font-black uppercase">
          <span className="grid h-11 w-11 rotate-[-4deg] place-items-center rounded-lg border-[3px] border-[#15100c] bg-[#e9321b] text-[#fff7e4]">
            UF
          </span>
          {demo.name}
        </Link>
        <div className="flex gap-5 overflow-x-auto text-sm font-black uppercase">
          <a href="#menu">Menu</a>
      ...
      >
        ...
        <div className="absolute right-[-74px] top-40 z-10 rotate-12 border-4 border-[#15100c] bg-[#e9321b] px-24 py-4 text-lg font-black uppercase text-[#fff7e4] shadow-[8px_8px_0_#15100c] max-md:hidden">
          Smash hot. Serve fast. Stay loud.
        </div>
        <div className="relative z-10 mx-auto w-[min(1120px,calc(100%-32px))] py-24">
          <p className="mb-3 text-sm font-black uppercase text-[#ffcf2e]">{demo.label}</p>
          <h1 className="max-w-3xl text-[clamp(3.2rem,10vw,7.8rem)] font-black uppercase leading-[0.84] text-[#fff7e4] [text-shadow:8px_8px_0_#15100c]">
            {demo.name}
          </h1>
          ...
          <div className="my-7 flex flex-wrap gap-3">
            {[`Rating ${demo.rating}`, demo.price, "Burgers / Fries / Chicken"].map((item) => (
              <span key={item} className="border-[3px] border-[#15100c] bg-[#fff7e4] px-4 py-2 font-black uppercase text-[#15100c] shadow-[4px_4px_0_#15100c]">
                {item}
              </span>
            ))}
          </div>
      ...
      <section className="border-y-[5px] border-[#15100c] bg-[#e9321b] py-14">
        <div className="mx-auto grid w-[min(1120px,calc(100%-32px))] gap-7 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          <div>
            <p className="text-sm font-black uppercase text-[#ffcf2e]">Lunch break fuel</p>
            <h2 className="mt-3 text-[clamp(2rem,5vw,4rem)] font-black uppercase leading-[0.95]">
              Combos, loaded fries, crispy chicken, and late-night bites.
            </h2>
          </div>
          <div className="flex flex-wrap gap-3">
            {demo.highlights.concat(["Burgers", "Fries", "Chicken", "Drinks"]).map((item) => (
              <span key={item} className="border-[3px] border-[#15100c] bg-[#ffcf2e] px-4 py-3 font-black uppercase text-[#15100c] shadow-[5px_5px_0_#15100c]">
                {item}
              </span>
            ))}
          </div>
      ...
      <section id="menu" className="mx-auto w-[min(1120px,calc(100%-32px))] py-20">
        <div className="mb-9 max-w-3xl">
          <p className="text-sm font-black uppercase text-[#ffcf2e]">Smash board</p>
          <h2 className="mt-3 text-[clamp(2rem,5vw,3.8rem)] font-black uppercase leading-[0.95]">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {demo.menu.map((item, index) => (
            <article key={item.name} className="overflow-hidden border-4 border-[#15100c] bg-[#fff7e4] text-[#15100c] shadow-[8px_8px_0_#15100c]">
              ...
              >
                <span className="absolute left-3 top-3 border-[3px] border-[#15100c] bg-[#e9321b] px-3 py-2 text-sm font-black uppercase text-[#fff7e4]">
                  {item.tag}
                </span>
                <strong className="absolute bottom-2 right-3 text-5xl font-black text-[#ffcf2e] [text-shadow:4px_4px_0_#15100c]">
                  0{index + 1}
                </strong>
              </div>
              <div className="p-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <strong className="text-2xl font-black text-[#e9321b]">{item.price}</strong>
                </div>
      ...
      <section className="mx-auto w-[min(1120px,calc(100%-32px))] py-20">
        <div className="grid gap-7 border-4 border-[#15100c] bg-[#ffcf2e] p-[clamp(22px,4vw,36px)] text-[#15100c] shadow-[12px_12px_0_#15100c] lg:grid-cols-[0.88fr_1.12fr]">
          <div>
            <p className="text-sm font-black uppercase text-[#e9321b]">Fast casual, big flavor</p>
            <h2 className="mt-3 text-[clamp(2rem,5vw,4rem)] font-black uppercase leading-[0.95]">
          ...
          <dl className="grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={label} className="border-[3px] border-[#15100c] bg-[#fff7e4] p-4">
                <dt className="text-xs font-black uppercase text-[#e9321b]">{label}</dt>
      ...
      <section id="gallery" className="mx-auto w-[min(1120px,calc(100%-32px))] py-20">
        <div className="mb-9 max-w-3xl">
          <p className="text-sm font-black uppercase text-[#ffcf2e]">Inside the flame</p>
          <h2 className="mt-3 text-[clamp(2rem,5vw,3.8rem)] font-black uppercase leading-[0.95]">
          ...
          {demo.gallery.map((image, index) => (
            ...
            >
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-4 pt-16">
                <h3 className="font-black uppercase text-[#fff7e4]">Burger Scene 0{index + 1}</h3>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="reservation" className="bg-[#ffcf2e] py-20 text-[#15100c]">
        <div className="mx-auto grid w-[min(1120px,calc(100%-32px))] gap-7 lg:grid-cols-[0.82fr_1.18fr]">
          <div className="border-4 border-[#15100c] bg-[#fff7e4] p-8 shadow-[8px_8px_0_#15100c]">
            <p className="text-sm font-black uppercase text-[#e9321b]">Contact</p>
            <h2 className="mt-3 text-[clamp(2rem,5vw,3.8rem)] font-black uppercase leading-[0.95]">
          ...
          <form className="grid gap-4 border-4 border-[#15100c] bg-[#fff7e4] p-6 shadow-[8px_8px_0_#15100c] sm:grid-cols-2">
            {["Name", "Phone", "Date", "Time"].map((label) => (
              <label key={label} className="grid gap-2 text-sm font-black uppercase text-[#e9321b]">
                {label}
                <input className="border-[3px] border-[#15100c] bg-[#fff7e4] px-4 py-3 text-[#15100c] outline-none" placeholder={label} />
              </label>
            ))}
            <label className="grid gap-2 text-sm font-black uppercase text-[#e9321b] sm:col-span-2">
              Notes
            ...
            <button type="button" className="border-[3px] border-[#15100c] bg-[#e9321b] px-5 py-4 font-black uppercase text-[#fff7e4] shadow-[6px_6px_0_#15100c] sm:col-span-2">
              Submit Demo Request
            </button>
          </form>
        </div>
      ...
      <footer id="location" className="border-t-[5px] border-[#15100c] bg-[#ffcf2e] py-12 text-[#15100c]">
        <div className="mx-auto grid w-[min(1120px,calc(100%-32px))] gap-7 md:grid-cols-3">
          <div>
            <h2 className="text-3xl font-black uppercase">{demo.name}</h2>
            <p className="mt-3 font-sans font-bold">Preview website. Not for commercial use.</p>
          </div>
          <div>
            <h3 className="font-black uppercase text-[#e9321b]">Location</h3>
            <p className="mt-2 font-sans font-bold">{demo.city}</p>
          </div>
          <div>
            <h3 className="font-black uppercase text-[#e9321b]">Hours</h3>
            <p className="mt-2 font-sans font-bold">Mon - Sun: 11:00 AM - 11:00 PM</p>
          </div>
        </div>
      </footer>

      <div className="fixed bottom-4 right-4 z-50 max-w-[min(310px,calc(100%-28px))] border-[3px] border-[#15100c] bg-[#ffcf2e] px-4 py-2 text-xs font-black uppercase text-[#15100c]">
